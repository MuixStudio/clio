import { fetchWithRetry } from '@/utils/run-safe'
...
async function getNewAccessToken(timeout: number): Promise<void> {
    try {
        ...
        if ((isRefreshingSign && isRefreshingSign === '1' && isRefreshingSignAvailable(timeout)) || isRefreshing) {
            ...
            //
            // AT and RT are stored in HttpOnly cookies — the browser sends the RT
            // automatically via credentials: "include"; no manual read/write needed.
