...
type (
	...
	User struct {
		...
		Password    string `gorm:"column:password;type:varchar(100);comment:密码(身份提供商不是Clio时可以为空)"`
		CountryCode string `gorm:"column:email;type:varchar(60);comment:手机区号"`
		Phone       string `gorm:"column:email;type:varchar(60);comment:手机号"`
