...
import (
	...
	"github.com/muixstudio/clio/internal/web/metadata"
	"github.com/muixstudio/clio/internal/web/router"
	"github.com/muixstudio/clio/internal/web/svc"
	"github.com/muixstudio/clio/internal/web/svc/observability"
...
type App struct {
	cfg      *config.Config
	metadata *metadata.ServiceMetadata
	svcCtx   *svc.ServiceContext
	logger   log.Logger
	flags    *cmdFlags
}

// NewCommand 创建 run 命令
...
func initializeApp(ctx context.Context, flags *cmdFlags) (*App, error) {
	...
	log.Infof("Loaded service metadata: %s v%s", metadata.Name, metadata.Version)
...
func loadServiceMetadata() (*metadata.ServiceMetadata, error) {
	// 尝试从 service.json 加载
	meta, err := metadata.LoadServiceMetadata()
	if err != nil {
		log.Warnf("Failed to load service.json: %v, using defaults", err)
		return metadata.LoadServiceMetadataOrDefault(), nil
	}
	return meta, nil
}
...
func (app *App) createKratosApp() (*kratos.App, error) {
	...
	// 3. 创建 Kratos 应用（使用 service.json 中的元数据）
	kratosApp := kratos.New(
		kratos.Name(app.metadata.Name),
		kratos.Version(app.metadata.Version),
	...
	log.Infof("Service: %s v%s (%s)",
		app.metadata.Name,
		app.metadata.Version,
		app.metadata.Description,
	)
	log.Infof("HTTP server will listen on %s", addr)
